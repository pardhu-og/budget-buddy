...
export default function Summary ({incview="true", expview="true", balview="true"}){
    ...
    return (
        <div className="flex flex-col items-center text-sm font-semibold m-1
                        md:text-base md:m-2
                        lg:text-xl lg:m-1">
            ...
            <p className={`${balview === "true" ? "flex":"hidden"}`}>Total Balance : <span className={`${balance <=0 ? "text-red-500": "text-green-600"} font-bold `}> {balance}</span></p>
        </div>
    )
}